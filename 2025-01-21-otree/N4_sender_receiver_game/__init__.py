...
class Constants(BaseConstants):
    ...
    POOL_SIZE = 4  # Each pool has 4 players
    TOTAL_PLAYERS = 16
    ...
    O_payoff_PA = ['By making Player B guess the secret number', 'By making Player B guess as high as possible', 'By making Player B guess close to the sent number', 'By making Player B guess higher than the sent number'] 
    ...
    O_payoff_other_PB = ['By making you guess the secret number', 'By making you guess as high as possible', 'By making you guess close to the sent number', 'By making you guess higher than the sent number'] 
    ...
    # Templates
    ReceiverReminder = 'N4_sender_receiver_game/templates/ReceiverReminder.html'
    SenderReminder = 'N4_sender_receiver_game/templates/SenderReminder.html'
...
class ControlQuestions(Page):
    ...
    form_model = 'player'
    form_fields = ['Q_task', 'Q_payoff', 'Q_payoff_other', 'Q_independence', 'Q_no_knowledge_guess',
                   'Q_secret_number_generation']
    ...
    def before_next_page(player, timeout_happened):
        ...
        if player.participant.role == 'Player A':
            ...
            c, player.Q_payoff_other_count = check_cq_answers(player.Q_payoff_other, Constants.A_payoff_other_PA,
                                                              player.Q_payoff_other_count)
        else:
            ...
            c, player.Q_payoff_other_count = check_cq_answers(player.Q_payoff_other, Constants.A_payoff_other_PB,
                                                              player.Q_payoff_other_count)
        d, player.Q_independence_count = check_cq_answers(player.Q_independence, Constants.A_independence,
                                                          player.Q_independence_count)
        e, player.Q_no_knowledge_guess_count = check_cq_answers(player.Q_no_knowledge_guess,
                                                                Constants.A_no_knowledge_guess,
                                                                player.Q_no_knowledge_guess_count)
        f, player.Q_secret_number_generation_count = check_cq_answers(player.Q_secret_number_generation,
                                                                      Constants.A_secret_number_generation,
                                                                      player.Q_secret_number_generation_count)
...
class TutorialIntro(Page):
    ...
    def before_next_page(player: Group, timeout_happened):
        ...
        if player.participant.treatment == "Decode":
